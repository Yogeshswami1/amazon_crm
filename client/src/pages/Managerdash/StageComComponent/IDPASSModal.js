...
const IDPASSModal = ({ visible, onCancel, record, fetchData }) => {
  const [id, setId] = useState(record.idAndPassCom?.id || '');
  const [pass, setPass] = useState(record.idAndPassCom?.pass || '');
  const [date, setDate] = useState(record.idAndPassCom?.date ? moment(record.idAndPassCom.date) : null);
